...
if __name__=='__main__' and args.start:
    make_data()
    print("shape of training set" + str(data_tr.shape))
    make_model()
    make_optimizer()
    train()
...
#     beta_input = model.decoder.weight.data
#     print_top_words(beta_input, list(zip(*sorted(vocab.items(), key=lambda x:x[1])))[0])
    print_perp(model)
    visualize()
